...
for(var book in OLD_TESTAMENT){
  if(OLD_TESTAMENT.hasOwnProperty(book)){
    form.book.insertAdjacentHTML('beforeend', "<option name='book' value='"+OLD_TESTAMENT[book]+"'>"+OLD_TESTAMENT[book]+"</option>");
  }
}
form.book.insertAdjacentHTML('beforeend', '<option disabled name="book" value="">(New Testament)</option>');
for(var book in NEW_TESTAMENT){
  if(NEW_TESTAMENT.hasOwnProperty(book)){
    form.book.insertAdjacentHTML('beforeend', "<option name='book' value='"+NEW_TESTAMENT[book]+"'>"+NEW_TESTAMENT[book]+"</option>");
  }
}
